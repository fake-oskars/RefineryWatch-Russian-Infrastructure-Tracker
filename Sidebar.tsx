...
// Dedicated Tweet Widget Component to handle lifecycle properly
const TweetWidget = ({ tweetId }: { tweetId: string }) => {
  ...
  useEffect(() => {
    ...
    if (w.twttr?.widgets) {
      w.twttr.widgets.createTweet(
        ...
        { theme: 'dark', conversation: 'none', dnt: true, align: 'center' }
      ).then((el: any) => {
        // If component unmounted or tweetId changed while creating, remove the widget
        if (!isMounted && el) {
          el.remove();
        }
      });
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-3">
              <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.5)]"></span>
              REFINERY<span className="text-slate-500">WATCH</span>
            </h1>
            <p className="text-xs text-slate-500 font-mono mt-1 tracking-wider">RUSSIAN INFRASTRUCTURE TRACKER</p>
          </div>
